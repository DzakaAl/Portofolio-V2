...
interface PreloaderProps {
  onComplete?: () => void;
  apiPromises?: Promise<unknown>[];
}

export default function Preloader({ onComplete, apiPromises = [] }: PreloaderProps) {
  ...
  useEffect(() => {
    ...
    const loadVideo = (videoEl: HTMLVideoElement): Promise<void> => {
    ...
    const loadAudio = (srcOrAudio: string | HTMLAudioElement): Promise<unknown> => {
      return new Promise<void>((resolve) => {
        const audio =
          typeof srcOrAudio === 'string' ? new Audio(srcOrAudio) : srcOrAudio;
    ...
    const startPreloading = async (): Promise<void> => {
      ...
      const trackSinglePromise = (promise: Promise<unknown>): Promise<unknown> => {
        return promise
          .then((res) => {
            if (isMounted) {
              completedItems += 1;
      ...
      // Emergency release only — normally we wait for every asset & API call
