...
interface AdminTechStackProps {
  onShowMessage: (type: 'success' | 'error', text: string) => void;
  onTechStacksCountChange?: (count: number) => void;
}
...
export default function TechStack({ onShowMessage, onTechStacksCountChange }: AdminTechStackProps) {
  ...
  // Form State
  const [stackForm, setStackForm] = useState<StackFormState>({
    title: '',
    alt: '',
    src: '',
    order: 0,
  ...
  const handleOpenEditModal = (stack: TechStack) => {
  ...
  const handleSubmitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      onShowMessage('error', (err as Error).message || 'Gagal menyimpan tech stack');
    } finally {
      setLoading(false);
    }
  };
